...
export default async function AdminOrdersPage() {
  ...
  // 1. Fetch orders with technical metadata
  // We use '*' to ensure we get 'payment_method' and 'status'
  const { data: orders, error } = await supabase
    ...
    `)
    // Fallback sort: using 'id' or 'updated_at' if 'created_at' is missing in your schema
  ...
  if (error) {
    ...
    return (
      <div className="p-10 text-center bg-red-50 rounded-[2rem] border border-red-100">
        <h2 className="text-red-600 font-black uppercase tracking-widest">Connection Interrupted</h2>
        <p className="text-red-400 text-sm font-mono mt-2">{error.message}</p>
      </div>
    )
  }

  // 2. Telemetry Calculations for the Header
  const pendingCash = orders?.filter(o => o.status === 'pending_cash').length || 0
  const totalProcessing = orders?.filter(o => o.status === 'paid' || o.status === 'shipped').length || 0
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium font-mono text-xs mt-1">
            System_Status: <span className="text-blue-600 tracking-widest uppercase">Fulfillment_Active</span>
          </p>
        </div>
